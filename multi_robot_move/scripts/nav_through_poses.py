    ...
    while tasks_master_robots[nav_master.getNameRobot()]:
        ...
        if name_first_slave == name_slave:
            ...
            while not nav_master.isTaskComplete():
                ...
                if feedback:
                    print("Feedback del maestro: " + str(feedback))
                    time_sec = Duration(seconds=90.0).nanoseconds / 1e9

                    nav_time = nav_master.getTimeNav(feedback.navigation_time.sec, is_sec=True)
                    max_time = nav_master.getTimeNav(time_sec, is_sec=True)
                    ...
                    if feedback.navigation_time.sec > time_sec:
...
async def navigate_robot_slave(nav_slave, goal_poses_robot, nav_start, name_master):
    ...
    while not nav_slave.isTaskComplete():
        ...
        if feedback:
            time_sec = Duration(seconds=10.0).nanoseconds / 1e9

            nav_time = nav_slave.getTimeNav(feedback.navigation_time.sec, is_sec=True)
            max_time = nav_slave.getTimeNav(time_sec, is_sec=True)
            ...
            if feedback.navigation_time.sec > time_sec:
                ...
                routes_master = len(goal_poses_robot) - (feedback.number_of_poses_remaining) 
                nav_slave.cancelTask()
                
                print("Rutas restantes por completar: " + str(feedback.number_of_poses_remaining))
                if feedback.number_of_poses_remaining > 0:
                    ...
                    if nav_master.getFeedback() is None:
                        ...
                        tasks_master_robots[nav_master.getNameRobot()][nav_slave.getNameRobot()] = goal_poses_robot[routes_master:]
